...
// Portable entries (work on every client) and featured ones get a bonus.
import { TYPES } from './taxonomy.mjs';
...
export function search(catalog, query, limit = 20) {
  ...
  for (const e of catalog) {
    ...
    if (score) scored.push([score + (e.featured ? 2 : 0) + (TYPES[e.type]?.portable ? 3 : 0), e]);
  }
