...
export const CLIENTS = {
  'claude-code': {
    name: 'Claude Code', tested: true, instructionsTarget: 'CLAUDE.md', docs: 'https://code.claude.com/docs/en/mcp', skillsAgent: 'claude-code', instructions: 'CLAUDE.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      if (s) {
        const env = envPairs(s.env).map(([k, v]) => `--env ${k}=${q(v)} `).join('');
        // Name before --env: the flag is variadic and would swallow the name (verified with Claude Code).
        return { kind: 'cli', lang: 'bash', text: `claude mcp add --transport stdio ${n} ${env}-- ${argv(s).map(q).join(' ')}` };
      }
  ...
  codex: {
    name: 'Codex CLI', tested: true, instructionsTarget: 'AGENTS.md', docs: 'https://developers.openai.com/codex/mcp', skillsAgent: 'codex', instructions: 'AGENTS.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      if (s) {
        const env = envPairs(s.env).map(([k, v]) => `--env ${k}=${q(v)} `).join('');
  ...
  'gemini-cli': {
    name: 'Gemini CLI', tested: true, instructionsTarget: 'GEMINI.md', docs: 'https://github.com/google-gemini/gemini-cli/blob/main/docs/tools/mcp-server.md', skillsAgent: 'gemini-cli', instructions: 'GEMINI.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      if (s) {
  ...
  cursor: {
    name: 'Cursor', instructionsTarget: 'AGENTS.md', docs: 'https://cursor.com/docs/context/mcp', skillsAgent: 'cursor', instructions: 'AGENTS.md / .cursor/rules',
  ...
  vscode: {
    name: 'VS Code (Copilot)', instructionsTarget: '.github/copilot-instructions.md', docs: 'https://code.visualstudio.com/docs/copilot/customization/mcp-servers', skillsAgent: 'github-copilot', instructions: '.github/copilot-instructions.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      const o = s
  ...
  opencode: {
    name: 'OpenCode', instructionsTarget: 'AGENTS.md', docs: 'https://opencode.ai/docs/mcp-servers/', skillsAgent: 'opencode', instructions: 'AGENTS.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      const o = s
  ...
  cline: {
    name: 'Cline', instructionsTarget: '.clinerules/', docs: 'https://docs.cline.bot/mcp/configuring-mcp-servers', skillsAgent: 'cline', instructions: '.clinerules',
  ...
  windsurf: {
    name: 'Windsurf', instructionsTarget: 'AGENTS.md', docs: 'https://docs.windsurf.com/windsurf/cascade/mcp', skillsAgent: 'windsurf', instructions: 'AGENTS.md',
  ...
  zed: {
    name: 'Zed', instructionsTarget: 'AGENTS.md', docs: 'https://zed.dev/docs/ai/mcp', skillsAgent: 'zed', instructions: 'AGENTS.md',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      const o = s
  ...
  goose: {
    name: 'Goose', instructionsTarget: '.goosehints', docs: 'https://block.github.io/goose/docs/getting-started/using-extensions', skillsAgent: 'goose', instructions: '.goosehints',
    mcp(e) {
      const n = serverName(e), { stdio: s, http: h } = e.mcp;
      const lines = s
  ...
  kiro: {
    name: 'Kiro', instructionsTarget: '.kiro/steering/', docs: 'https://kiro.dev/docs/mcp/configuration/', skillsAgent: 'kiro-cli', instructions: '.kiro/steering/',
  ...
  roo: {
    name: 'Roo Code', instructionsTarget: '.roo/rules/', docs: 'https://docs.roocode.com/features/mcp/using-mcp-in-roo', skillsAgent: 'roo', instructions: '.roo/rules/ + AGENTS.md',
  ...
  dsh: {
    name: 'DeepSeek Harness', instructionsTarget: 'AGENTS.md', docs: 'https://github.com/deepseek-ai/deepseek-harness/blob/main/docs/user/guide/mcp-memory.md',
...
// Agent instructions: append to the client's instructions file, or drop a file
// into its rules directory when the target ends with "/".
export function instructionsInstall(e, clientId) {
  const target = CLIENTS[clientId]?.instructionsTarget;
  if (!target) return null;
  const url = q(e.instructions.url);
  if (target.endsWith('/')) {
    const file = `${target}${e.id}.md`;
    return { kind: 'cli', lang: 'bash', text: `mkdir -p ${target.slice(0, -1)} && curl -fsSL ${url} -o ${file}` };
  }
  return { kind: 'cli', lang: 'bash', text: `curl -fsSL ${url} >> ${target}` };
}
...
export function installFor(e, clientId) {
  ...
  if (e.type === 'instructions') return instructionsInstall(e, clientId);
...
export function clientsFor(e) {
  if (e.type === 'mcp-server' || e.type === 'skill-pack' || e.type === 'instructions') return CLIENT_IDS;
