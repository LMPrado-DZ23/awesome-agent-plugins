...
export function validateEntry(e) {
  ...
  if (e.alternatives !== undefined) {
    if (!Array.isArray(e.alternatives) || e.alternatives.length > 4) errs.push('alternatives: up to 4');
      ...
      if (!isObj(a)) {
        errs.push(`alternatives[${i}]: must be a mapping`);
        return;
      }
...
export function validateCatalog(entries) {
  ...
  for (const { file, entry } of entries) {
    ...
    const alternatives = Array.isArray(entry.alternatives) ? entry.alternatives.filter(isObj) : [];
    for (const u of [entry.url, ...alternatives.map((a) => a.url)]) {
      if (typeof u !== 'string' || !u) continue;
      const k = norm(u);
      if (urls.has(k)) errs.push(`${file}: ${u} already listed in ${urls.get(k)}`);
      urls.set(k, file);
    }
  }
  return errs;
...
export function validateCollections(collections, entryIds) {
  ...
  for (const { file, collection: c } of collections) {
    if (!isObj(c)) {
      errs.push(`${file}: collection must be a mapping`);
      continue;
    }
